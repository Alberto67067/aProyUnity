...
public class MiniMAze5x5 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Vector3 posk = new Vector3(mapInic.ENTRDS[0].Item1 - 2,mapInic.ENTRDS[0].Item2 - 2);
    ...
    {
        if(Math.Abs(Input.GetAxisRaw("Horizontal")) == 1 && PosicionValida(matriz, (int)Input.GetAxisRaw("Horizontal") + 2 + (int)Knight.transform.position.x, (int)Knight.transform.position.y + 2, false))
        {
            move = new Vector2(Knight.transform.position.x + Input.GetAxisRaw("Horizontal"), Knight.transform.position.y);
        }
        else if(Math.Abs(Input.GetAxisRaw("Vertical")) == 1 && PosicionValida(matriz, (int)Knight.transform.position.x + 2, (int)Input.GetAxisRaw("Vertical") + 2 + (int)Knight.transform.position.y, false))
        {
