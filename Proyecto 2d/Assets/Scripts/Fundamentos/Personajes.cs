...
namespace Heroes
{
    ...
    /// 
    /// </summary>
    [Serializable] public class Heroe// De aqui en adelante en donde sea que este escrito "Personaje" me refiero a Heroe, es que el nombre este lo cambie despues de haberlo hecho todo
    //Perdon, a quien este leyendo mi codigo.
    {
        ...
        public bool EstaEnRango(Heroe Target, Tipo.Poder a)
        {
            ...
            while(costo < a.rango)    
            {
                for (int k = 0; k < 4; k++)
                {
                    if(map.PosicionValida(map.MAP, x[k] + dx[k], y[k] + dy[k],true) && map.MAP[x[k] + dx[k], y[k] + dy[k]] != 1)
                    {
                        x[k] += dx[k];
                        y[k] += dy[k];
                    }
        ...
        [Serializable]public class Tipo
        {
            ...
            */
            ...
            [Serializable]public class AtacarPoder : Poder, IAttack
            {
                public int damage;
